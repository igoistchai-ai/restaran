...
async def login(data: LoginData):
    ...
    if not user:
        raise HTTPException(401, "Неверный номер или PIN")
...
async def admin_web_login(
    ...
    # Только этот Telegram ID является админом
...
async def admin_create_customer(
    ...
    cur = conn.execute(
        """
        INSERT INTO users
        (name,phone,pin_hash,pin_plain,role,created_at)
        VALUES (?,?,?,?,?,?)
        """,
        (
            data.name,
...
async def random_command(
    ...
    await update.message.reply_text(
        ...
        "который зарегистрирован у вас в RESTARAN.",
...
async def contact_handler(
    ...
    if not user:
        # Иногда Telegram присылает номер без +
