...
async def payment_config():
    return {"online_enabled": bool(ARCA_USER_NAME and ARCA_PASSWORD and ARCA_RETURN_URL), "provider": "ArCa / iPay", "commission_percent": CASH_COMMISSION_PERCENT}
...
async def direct_delivery(data: DirectDeliveryData, authorization: str = Header(default="")):
    ...
    payment=data.payment_method if data.payment_method in ("cash","online") else "cash"
    if payment=="online" and not (ARCA_USER_NAME and ARCA_PASSWORD and ARCA_RETURN_URL): raise HTTPException(503,"Онлайн-оплата не настроена. Администратору нужно добавить данные ArCa в Render.")
