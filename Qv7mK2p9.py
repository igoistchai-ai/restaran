...
async def promote_courier(customer_id: int, authorization: str = Header(default="")):
    ...
    conn.execute("UPDATE sessions SET role='courier' WHERE user_id=?", (customer_id,)); conn.commit(); conn.close(); log_action(customer_id, "promote_courier", ""); return {"ok":True,"role":"courier"}
...
async def courier_location(
    ...
    if not (-90 <= lat <= 90 and -180 <= lon <= 180):
        conn.close()
        raise HTTPException(400, "Некорректные координаты GPS")
    now=int(time.time())
    accuracy=max(0.0,float(data.accuracy or 0))
    speed=max(0.0,float(data.speed or 0))
    heading=None if data.heading is None else float(data.heading)

    # Текущая позиция всегда перезаписывается свежей точкой.
    conn.execute("""
        UPDATE couriers
        SET lat=?, lon=?, updated_at=?
        WHERE id=?
    """, (lat,lon,now,courier["id"]))
    # История нужна для диагностики GPS и никогда не используется вместо current position.
    conn.execute("""
        INSERT INTO courier_locations(courier_id,lat,lon,accuracy,speed,heading,created_at)
        VALUES(?,?,?,?,?,?,?)
    """, (courier["id"],lat,lon,accuracy,speed,heading,now))
    # Ограничиваем историю последними ~2000 точками на курьера.
    conn.execute("""
        DELETE FROM courier_locations
        WHERE courier_id=? AND id NOT IN (
            SELECT id FROM courier_locations WHERE courier_id=? ORDER BY id DESC LIMIT 2000
        )
    """, (courier["id"],courier["id"]))

    conn.commit()
    conn.close()
    return {"ok": True, "lat":lat, "lon":lon, "accuracy":accuracy, "updated_at":now}
