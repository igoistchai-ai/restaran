...
async def admin_create_admin(data: AdminCreate, authorization: str = Header(default="")):
    require_admin(authorization)
    phone = normalize_phone(data.phone)
    if len(phone) < 5:
...
async def payment_config():
    return {"online_enabled": bool(ARCA_USER_NAME and ARCA_PASSWORD and ARCA_RETURN_URL), "provider": "Наличные", "commission_percent": CASH_COMMISSION_PERCENT}

def arca_register(amount, order_number, description):
    if not (ARCA_USER_NAME and ARCA_PASSWORD and ARCA_RETURN_URL):
        raise HTTPException(503, "Онлайн-оплата ещё не подключена: добавьте ARCA_USER_NAME, ARCA_PASSWORD и ARCA_RETURN_URL в Render Environment.")
    data = urlencode({"userName": ARCA_USER_NAME, "password": ARCA_PASSWORD, "orderNumber": order_number, "amount": str(int(round(amount*100))), "returnUrl": ARCA_RETURN_URL, "description": description}).encode()
    req = Request(f"{ARCA_BASE_URL}/register.do", data=data, headers={"Content-Type":"application/x-www-form-urlencoded"}, method="POST")
    try:
        with urlopen(req, timeout=15) as resp:
            payload=json.loads(resp.read().decode("utf-8", "ignore"))
    except Exception as exc:
        raise HTTPException(502, f"Платёжный шлюз недоступен: {exc}")
    if payload.get("errorCode") not in (None, 0, "0") or not payload.get("formUrl"):
        raise HTTPException(502, payload.get("errorMessage") or "Не удалось создать платёж")
    return payload

@app.get("/api/courier/finance")
async def courier_finance(authorization: str = Header(default="")):
    user=require_user(authorization)
    if user["role"]!="courier": raise HTTPException(403,"Нет доступа")
    conn=db(); courier=conn.execute("SELECT * FROM couriers WHERE user_id=?",(user["user_id"],)).fetchone()
    if not courier: conn.close(); raise HTTPException(404,"Курьер не найден")
    rows=conn.execute("SELECT * FROM courier_commissions WHERE courier_id=? AND status='due' ORDER BY id",(courier["id"],)).fetchall(); conn.close()
    due=sum(float(r["amount_amd"] or 0) for r in rows)
    return {"due":due,"commission_percent":CASH_COMMISSION_PERCENT,"service_payment_account":SERVICE_PAYMENT_ACCOUNT,"items":[dict(r) for r in rows]}

@app.post("/api/courier/commission/pay")
async def courier_commission_pay(authorization: str = Header(default="")):
    user=require_user(authorization)
    if user["role"]!="courier": raise HTTPException(403,"Нет доступа")
    conn=db(); courier=conn.execute("SELECT * FROM couriers WHERE user_id=?",(user["user_id"],)).fetchone()
    if not courier: conn.close(); raise HTTPException(404,"Курьер не найден")
    rows=conn.execute("SELECT * FROM courier_commissions WHERE courier_id=? AND status='due' ORDER BY id",(courier["id"],)).fetchall(); due=sum(float(r["amount_amd"] or 0) for r in rows)
    if due<=0: conn.close(); return {"ok":True,"due":0}
    order_number=f"SDC{courier["id"]}{int(time.time())}"
    payload=arca_register(due,order_number,f"SERTAL commission courier {courier["id"]}")
    provider_id=str(payload.get("orderId") or order_number)
    conn.execute("UPDATE courier_commissions SET payment_order_number=? WHERE courier_id=? AND status='due'",(provider_id,courier["id"])); conn.commit(); conn.close()
    return {"ok":True,"due":due,"payment_url":payload["formUrl"],"provider_order_id":provider_id,"order_number":order_number}

@app.get("/api/payment/return")
async def payment_return(orderId: str = "", MDOrder: str = ""):
    ext=orderId or MDOrder
    if not ext: return RedirectResponse("/")
    # ArCa may return MDOrder; verify by getOrderStatus before changing balances.
    verified=False; status=None
    if ARCA_USER_NAME and ARCA_PASSWORD:
        q=urlencode({"userName":ARCA_USER_NAME,"password":ARCA_PASSWORD,"orderId":ext}).encode()
        try:
            with urlopen(Request(f"{ARCA_BASE_URL}/getOrderStatus.do",data=q,headers={"Content-Type":"application/x-www-form-urlencoded"},method="POST"),timeout=15) as resp:
                st=json.loads(resp.read().decode("utf-8","ignore")); status=st.get("orderStatus"); verified=str(status) in {"2","6"}
        except Exception: pass
    conn=db()
    if verified:
        tx=conn.execute("SELECT id FROM orders WHERE payment_order_number=?",(ext,)).fetchone()
        if tx: conn.execute("UPDATE orders SET payment_status='paid' WHERE payment_order_number=?",(ext,))
        conn.execute("UPDATE courier_commissions SET status='paid',paid_at=? WHERE payment_order_number=? AND status='due'",(int(time.time()),ext))
        conn.commit()
    conn.close()
    return RedirectResponse(f"/?payment={'success' if verified else 'pending'}")

@app.post("/api/customer/orders/direct")
async def direct_delivery(data: DirectDeliveryData, authorization: str = Header(default="")):
    user=require_user(authorization)
    if user["role"]!="customer": raise HTTPException(403,"Нет доступа")
    from_address=str(data.from_address or "").strip(); address=str(data.address or "").strip(); title=str(data.title or "").strip()
    if not from_address or not address or not title: raise HTTPException(400,"Укажите откуда, куда и что доставить")
    weight=float(data.weight_kg or 0); price=float(data.price or 0)
    if weight<0 or weight>MAX_ORDER_WEIGHT_KG: raise HTTPException(400,f"Вес должен быть от 0 до {MAX_ORDER_WEIGHT_KG:g} кг")
    if price<=0: raise HTTPException(400,"Введите сумму заказа")
    payment=data.payment_method if data.payment_method in ("cash","online") else "cash"
    if payment=="online" and not (ARCA_USER_NAME and ARCA_PASSWORD and ARCA_RETURN_URL): raise HTTPException(503,"Онлайн-оплата временно недоступна")
    flat,flon=data.from_lat,data.from_lon; tlat,tlon=data.to_lat,data.to_lon
    if flat is None or flon is None: flat,flon=geocode_yerevan(from_address)
    if tlat is None or tlon is None: tlat,tlon=geocode_yerevan(address)
    conn=db()
    promo_code=str(data.promo_code or "").strip().upper(); discount=0.0
    if promo_code:
        promo=conn.execute("SELECT * FROM promos WHERE code=? AND active=1",(promo_code,)).fetchone()
        if not promo: conn.close(); raise HTTPException(400,"Промокод не найден или выключен")
        if price<float(promo["min_order"] or 0): conn.close(); raise HTTPException(400,f"Минимальная сумма: {float(promo['min_order']):.0f} AMD")
        if promo["max_uses"] is not None and int(promo["used_count"] or 0)>=int(promo["max_uses"]): conn.close(); raise HTTPException(400,"Лимит промокода исчерпан")
        discount=min(price,float(promo["value"]) if promo["kind"]=="fixed" else price*float(promo["value"])/100)
    total=max(0,price-discount); bonus=calculate_courier_bonus(weight)
    payment_number=f"SDO{int(time.time()*1000)}" if payment=="online" else None
    cur=conn.execute("""INSERT INTO orders(customer_id,title,address,price,status,created_at,weight_kg,courier_bonus_amd,lat,lon,restaurant_address,restaurant_lat,restaurant_lon,recipient_name,delivery_note,payment_method,payment_status,payment_note,promo_code,discount_amd,payment_order_number) VALUES(?,?,?,?,'new',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",(user["user_id"],title,address,total,int(time.time()),weight,bonus,tlat,tlon,from_address,flon,flat,data.recipient_name.strip(),data.delivery_note.strip(),payment,"pending" if payment=="online" else "cash","Пожалуйста, подготовьте сумму без сдачи. Курьер может не иметь размена." if payment=="cash" else "Онлайн-оплата",promo_code or None,discount,payment_number))
    if promo_code: conn.execute("UPDATE promos SET used_count=used_count+1 WHERE code=?",(promo_code,))
    conn.commit(); order_id=cur.lastrowid
    conn.close()
    if payment=="online":
        payload=arca_register(total,payment_number,f"SERTAL order #{order_id}")
        conn=db(); conn.execute("UPDATE orders SET payment_order_number=? WHERE id=?",(str(payload.get("orderId") or payment_number),order_id)); conn.commit(); conn.close()
        return {"ok":True,"order_id":order_id,"total":total,"discount":discount,"payment_url":payload["formUrl"],"payment_method":"online"}
    return {"ok":True,"order_id":order_id,"total":total,"discount":discount,"payment_method":"cash","cash_notice":"Пожалуйста, платите без сдачи."}
...
@app.get("/api/customer/promos")
async def customer_promos(authorization: str = Header(default="")):
    user=require_user(authorization)
    if user["role"]!="customer": raise HTTPException(403,"Нет доступа")
    conn=db(); rows=conn.execute("SELECT code,kind,value,min_order,max_uses,used_count FROM promos WHERE active=1 AND (max_uses IS NULL OR used_count<max_uses) ORDER BY id DESC").fetchall(); conn.close()
...
async def promote_courier(customer_id: int, authorization: str = Header(default="")):
    ...
    # Existing customer sessions must immediately become courier sessions.
    # Do not call the old undefined log_action() helper here: it caused a 500
    # after the database transaction had already been committed.
    conn.execute("UPDATE sessions SET role='courier' WHERE user_id=?", (customer_id,))
    conn.commit()
    conn.close()
    return {"ok": True, "role": "courier"}
...
async def courier_orders(
    ...
    if not courier or not courier["active"]:
        conn.close()
        raise HTTPException(403, "Курьер деактивирован")

    due = conn.execute("SELECT COALESCE(SUM(amount_amd),0) AS due FROM courier_commissions WHERE courier_id=? AND status='due'", (courier["id"],)).fetchone()["due"]
    if float(due or 0) > 0:
        conn.close()
        raise HTTPException(402, f"Доступ к заказам заблокирован. Комиссия к оплате: {float(due):.0f} AMD")
...
async def courier_online(data: OnlineData, authorization: str = Header(default="")):
    user=require_user(authorization)
    if user["role"]!="courier": raise HTTPException(403,"Нет доступа")
    conn=db(); courier=conn.execute("SELECT * FROM couriers WHERE user_id=? AND active=1",(user["user_id"],)).fetchone()
    if not courier: conn.close(); raise HTTPException(403,"Курьер не активен")
    if data.online:
        now=int(time.time())
        if not courier["updated_at"] or now-int(courier["updated_at"])>120 or courier["lat"] is None or courier["lon"] is None:
            conn.close(); raise HTTPException(409,"Для выхода на линию необходимо разрешить постоянную геопозицию")
    conn.execute("UPDATE couriers SET online=? WHERE id=?",(1 if data.online else 0,courier["id"]))
    conn.commit();conn.close()
    await send_work_log(f"Курьер #{courier['id']} {'вышел на линию' if data.online else 'завершил работу'}.")
    return {"ok":True,"online":bool(data.online)}
...
async def courier_accept(
    ...
    await broadcast_location_event({"type":"order","order_id":int(order_id),"status":"accepted"})
    await send_work_log(f"Заказ #{order_id} принят курьером.")
    return {"ok": True}
...
async def courier_start(
    ...
    await broadcast_location_event({"type":"order","order_id":int(order_id),"status":"delivering"})
    await send_work_log(f"Заказ #{order_id}: курьер начал доставку.")
    return {"ok": True}
...
async def courier_complete(
    ...
    order = conn.execute("SELECT * FROM orders WHERE id=? AND courier_id=?", (order_id, courier["id"])).fetchone()
    if not order or order["status"] != "delivering":
        conn.close(); raise HTTPException(400, "Нельзя завершить эту доставку")
    cur = conn.execute("""
        UPDATE orders SET status='delivered', delivered_at=?
        WHERE id=? AND courier_id=? AND status='delivering'
    """, (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), order_id, courier["id"]))
    commission = 0.0
    if cur.rowcount and str(order["payment_method"] or "cash") == "cash":
        commission = round(float(order["price"] or 0) * CASH_COMMISSION_PERCENT / 100.0, 2)
        if commission > 0:
            conn.execute("INSERT OR IGNORE INTO courier_commissions(courier_id,order_id,amount_amd,status,created_at) VALUES(?,?,?,?,?)", (courier["id"], order_id, commission, "due", int(time.time())))
    conn.commit()
    conn.close()
    ...
    await broadcast_location_event({"type":"order","order_id":int(order_id),"status":"delivered"})
    await send_work_log(f"Заказ #{order_id} доставлен.")
    return {"ok": True}


@app.post("/api/courier/orders/{order_id}/complete-with-photo")
async def courier_complete_with_photo(order_id:int,file:UploadFile=File(...),authorization:str=Header(default="")):
    user=require_user(authorization)
    if user["role"]!="courier": raise HTTPException(403,"Нет доступа")
    if not (file.content_type or "").startswith("image/"): raise HTTPException(400,"Нужно фото")
    raw=await file.read()
    if len(raw)>10*1024*1024: raise HTTPException(413,"Фото больше 10 МБ")
    conn=db();courier=conn.execute("SELECT * FROM couriers WHERE user_id=? AND active=1",(user["user_id"],)).fetchone()
    order=conn.execute("SELECT * FROM orders WHERE id=? AND courier_id=?",(order_id,courier["id"] if courier else -1)).fetchone() if courier else None
    if not order or order["status"]!="delivering": conn.close();raise HTTPException(400,"Заказ нельзя завершить")
    ext=Path(file.filename or "delivery.jpg").suffix.lower();ext=ext if ext in {".jpg",".jpeg",".png",".webp",".heic",".heif"} else ".jpg"
    name=f"delivery_proof_{order_id}_{secrets.token_hex(12)}{ext}";path=UPLOAD_DIR/name;path.write_bytes(raw)
    now=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    cur=conn.execute("UPDATE orders SET status='delivered',delivered_at=?,receipt_file=? WHERE id=? AND courier_id=? AND status='delivering'",(now,str(path),order_id,courier["id"]))
    if cur.rowcount==0: conn.close();raise HTTPException(400,"Заказ уже изменён")
    if str(order["payment_method"] or "cash")=="cash":
        commission=round(float(order["price"] or 0)*CASH_COMMISSION_PERCENT/100.0,2)
        if commission>0: conn.execute("INSERT OR IGNORE INTO courier_commissions(courier_id,order_id,amount_amd,status,created_at) VALUES(?,?,?,?,?)",(courier["id"],order_id,commission,"due",int(time.time())))
    customer=conn.execute("SELECT * FROM users WHERE id=?",(order["customer_id"],)).fetchone()
    conn.execute("INSERT INTO courier_chat_messages(order_id,customer_id,courier_id,sender_role,sender_user_id,text,file_name,file_url,created_at) VALUES(?,?,?,?,?,?,?,?,?)",(order_id,int(order["customer_id"]),int(courier["id"]),"courier",int(user["user_id"]),"Фото подтверждения доставки",file.filename or name,f"/api/courier-chat/media/{name}",int(time.time())))
    conn.commit();conn.close()
    await broadcast_location_event({"type":"order","order_id":int(order_id),"status":"delivered","proof_url":f"/api/customer/orders/{order_id}/proof"})
    await send_work_log(f"Заказ #{order_id} доставлен. Фото подтверждения сохранено.")
    if customer and customer["telegram_id"] and telegram_app:
        try:
            with open(path,"rb") as fh: await telegram_app.bot.send_photo(customer["telegram_id"],photo=fh,caption=f"SERTAL DELIVERY\nЗаказ №{order_id} доставлен.\nФото подтверждения доставки приложено.\nПодтвердите получение на сайте.")
        except Exception as exc: print("DELIVERY PHOTO TELEGRAM:",exc)
    return {"ok":True,"order_id":order_id,"proof_url":f"/api/customer/orders/{order_id}/proof"}
